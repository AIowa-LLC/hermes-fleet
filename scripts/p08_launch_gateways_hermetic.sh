...
TAIL_IP=<tailnet-ip>
PORT=9120
VENV_HERMES=<local-hermes-path>/hermes-agent/venv/bin/hermes
...
ENV_ARGS=()
if [ -f <local-hermes-path>/.env ]; then
...
launch() { # $1=bind-ip $2=log
  env -i \
    PATH="/usr/bin:/bin:/usr/sbin:/sbin:<local-hermes-path>/hermes-agent/venv/bin" \
    HOME=$HOME \
    HERMES_HOME=<local-hermes-path> \
...
N=$(count_listeners)
echo "listeners up: $N (want 2)"
[ "$N" -ge 2 ] || { echo "FAIL: gateways did not come up"; tail -5 "$LOG_LAN" "$LOG_TAIL"; exit 1; }
...
for pid in $(lsof -nP -iTCP:$PORT -sTCP:LISTEN -t 2>/dev/null | sort -u); do
  ...
  [ "$BAD" -ne 0 ] && FAIL=1
  echo "$HOMEV" | grep -q "HERMES_HOME=<local-hermes-path>$" || FAIL=1
done
