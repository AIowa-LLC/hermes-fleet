...
# A pull request or merge candidate can touch many product areas at once (for
# example, a cross-cutting networking/security change). Keep the signal bounded
# and deterministic: the full five-shard deep lane covers the complete
# inventory. Oversized selections still run a mandatory, conservative CORE
# journey set instead of timing out after an unbounded serial list of
...
if [ -z "$ORDERED" ]; then
  echo "UI-PREFLIGHT: no UI-relevant changes in this diff; skipping focused UI run (merge-group critical smoke and full regression remain separate checks)."
  exit 0
fi
