        ...
        VStack(alignment: .leading, spacing: 8) {
            DisclosureGroup(isExpanded: $expanded) {
                if let detail, !detail.isEmpty {
                    ...
                    Text("No additional output reported.").font(.caption).foregroundStyle(FleetTheme.textSecondary)
                }
            } label: {
                Label(title, systemImage: "wrench.and.screwdriver")
                    .font(.subheadline.weight(.medium)).foregroundStyle(FleetTheme.textPrimary)
