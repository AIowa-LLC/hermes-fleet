...
public actor KanbanEventStreamClient: KanbanBoardWatching {
    ...
    public func snapshot() async throws -> KanbanBoardSnapshot {
        var request = URLRequest(
            url: baseURL.appendingPathComponent("api/plugins/kanban/board"))
    ...
    private func pumpOneConnection() async throws {
        ...
        guard let url = Self.buildEventsURL(
            base: baseURL, since: lastCursor, authentication: authentication
        ) else {
    ...
    /// connection's auth query (`?ticket=` / `?token=`) — mirrors
    /// `GatewayWebSocketTransport.buildWebSocketURL` (the secret rides in
    /// the returned URL; never log it directly, spec §29).
