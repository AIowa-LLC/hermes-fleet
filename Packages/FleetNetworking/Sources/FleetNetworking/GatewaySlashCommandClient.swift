...
public struct GatewaySlashCommandClient: SlashCommandProviding {
    ...
    static func decodeSkillDispatch(
        ...
        let name = trimmedName.hasPrefix("/")
            ? String(trimmedName.dropFirst())
            : trimmedName
        guard RoutingGuard.isValidRouteComponent(name) else {
            throw SlashCommandError.malformedResponse("skill dispatch result has an unsafe 'name'")
        }
        let fallbackDisplay = "/\(name)" + (argument.isEmpty ? "" : argument.first?.isWhitespace == true ? argument : " \(argument)")
        ...
        return SkillCommandDispatch(name: name, message: message, display: display)
    }
