...
Privacy answers must describe the exact RC and its included dependencies, not this walkthrough's assumptions. The repository-side privacy manifest and required-reason API audit (issue #13, `HermesFleetApp/PrivacyInfo.xcprivacy`) have landed on `main`; what remains external is reconciling that audit with App Store Connect's privacy answers, the privacy policy, and the final submitted build. Before external submission, the release owner must complete that reconciliation.
