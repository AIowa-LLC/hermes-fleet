...
Deterministic mapping: `U7GatewayQrLockSettingsUITests`, `H1AppLockUITests` (re-admitted to hosted deterministic CI on 2026-09-13, PR #28: the earlier hosted failures were a test-harness cross-suite nav-leakage defect, not environmental).

Physical-device mapping: Face ID unlock/relock on real hardware is NOT covered by the hosted deterministic suite; the RC must exercise enable → background/foreground relock → Face ID unlock, cancel/fail, force-quit relaunch on the physical device below.
